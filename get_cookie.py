...
def save_dictionary_to_file(dictionary, filename):
    project_directory = os.getcwd()  # Получаем текущую директорию проекта
    file_path = os.path.join(project_directory, filename)  # Объединяем путь к файлу с текущей директорией

    with open(file_path, 'w') as file:
        json.dump(dictionary, file)  # Сохраняем словарь в файл
...
if __name__ == '__main__':
    ...
    save_dictionary_to_file(parse_cookies(cookie), 'avito_cookie.py')
